...
class question_generator {
    ...
    /**
     * Returns the default Mustache template for the system prompt.
     *
     * Supported placeholders: {{primer}}, {{instructions}}, {{example}}, {{story}},
     * {{existingquestionsjson}}, {{#sendexistingquestions}}, {{#mode_topic}}, {{#mode_story}}.
     *
     * @return string the default Mustache prompt template
     */
...
The question that will be generated by you has to be as different as possible from all of the following questions in this JSON string:
...
EOT;
    ...
    /**
     * Renders the given Mustache prompt template with the provided context.
     *
     * HTML escaping is disabled because the prompt is plain text, not HTML.
     *
     * @param string $template the Mustache template string
     * @param array $context associative array of context variables (keys: example, existingquestionsjson,
     *  instructions, mode_story, mode_topic, primer, sendexistingquestions, story)
     * @return string the rendered prompt
     */
    public static function render_prompt_template(string $template, array $context): string {
    ...
    public function generate_question(stdClass $dataobject, bool $sendexistingquestionsascontext): stdClass|string {
        ...
        if ($provider === 'local_ai_manager') {
            $template = get_config('qbank_questiongen', 'systemprompt');
            if (empty($template)) {
            ...
            $context = [
                ...
                'mode_story' => in_array($dataobject->mode, [
                    story_form::QUESTIONGEN_MODE_STORY,
                    story_form::QUESTIONGEN_MODE_COURSECONTENTS,
                ]),
                'mode_topic' => $dataobject->mode === story_form::QUESTIONGEN_MODE_TOPIC,
                'primer' => $primer,
                'sendexistingquestions' => !empty($existingquestionsjson),
                'story' => $story,
            ];

            $systemprompt = self::render_prompt_template($template, $context);
